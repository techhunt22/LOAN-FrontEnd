...
export const AddClientOption3Form = () => {
  ...
  const [isSameForAll, setIsSameForAll] = useState<boolean>(true);
  const [isDifferentForEach, setIsDifferentForEach] = useState<boolean>(false);
  const [showDifferentFields, setShowDifferentFields] =
    useState<boolean>(false);
  const [disputeList, setDisputeList] = useState<Dispute[]>([]);
  const [isEquifaxSelected, setIsEquifaxSelected] = useState<boolean>(false);
  const [isExperianSelected, setIsExperianSelected] = useState<boolean>(false);
  const [isTransUnionSelected, setIsTransUnionSelected] =
    useState<boolean>(false);
  ...
  const handleCheckboxChange = (
    ...
    if (bureau === "equifax") {
      setIsEquifaxSelected(isChecked);
    } else if (bureau === "experian") {
      setIsExperianSelected(isChecked);
    } else if (bureau === "transUnion") {
      setIsTransUnionSelected(isChecked);
    }
  };

  const handleNextClick = () => {
    setDisputeList((prevList) => [
      ...prevList,
      {
        creditor: creditorName,
        reason: reason,
        disputeItem: disputeDetail,
        accountno: accountnumber,
        equifaxStatus: isEquifaxSelected ? "Negative" : "",
        experianStatus: isExperianSelected ? "Negative" : "",
        transUnionStatus: isTransUnionSelected ? "Negative" : "",
      },
    ]);
    ...
    setExperianStatus("");
    setTransUnionStatus("");
    setIsEquifaxSelected(false);
    setIsExperianSelected(false);
    setIsTransUnionSelected(false);
  ...
  const handleSameForAll = () => {
    ...
    setIsDifferentForEach(false);
    setShowDifferentFields(false);
  };

  const handleDifferentForEach = () => {
    setIsSameForAll(false);
    setIsDifferentForEach(true);
    setShowDifferentFields(true);
  };
  ...
  const updateContent = (templateContent: string, bureau: string) => {
    ...
    const updatedContent = templateContent
      .replace("{client_first_name}", selectedUserData?.first_name || "")
  ...
  const handleDelete = (index: number) => {
    setDisputeList((prevList) => prevList.filter((_, i) => i !== index));
  };
    ...
    <>
      <section className="w-full h-full flex flex-col items-center justify-center pt-20  mb-20 ">
              ...
              <div className="w-[800px] h-max pb-10 bg-white">
                ...
                <div className="w-full h-full flex flex-col gap-6">
                  {disputeList.map((item, index) => (
                    ...
                    >
                      <p className="w-[20%]">{item.creditor || "N/A"}</p>
                      <p className="w-[20%]">Equifax, Exparian, Transunion</p>
                      <p className="w-[15%]">{item.reason || "N/A"}</p>
                      <p className="w-[13%] text-red-400">
                        {item.equifaxStatus ? "Negative" : ""}
                      </p>
                      <p className="w-[13%] text-red-400">
                        {item.experianStatus ? "Negative" : ""}
                      </p>
                      <p className="w-[13%] text-red-400">
                        {item.transUnionStatus ? "Negative" : ""}
                      </p>
        ...
        >
          <div
            ref={modalRef}
            className="w-[400px] md:w-[350px] h-[500px] bg-white flex flex-col items-center justify-center border-2 border-black"
          >
            ...
            <div className="w-full h-full flex gap-2">
              <div className="w-[50%] h-full flex flex-col">
                <div className="flex gap-2">
                  <input
                    type="checkbox"
                    checked={isEquifaxSelected}
                    onChange={(e) =>
                      handleCheckboxChange("equifax", e.target.checked)
                    }
                  />
                  <p>Equifax</p>
                </div>
                <div className="flex gap-2">
                  <input
                    type="checkbox"
                    checked={isExperianSelected}
                    onChange={(e) =>
                      handleCheckboxChange("experian", e.target.checked)
                    }
                  />
                  <p>Experian</p>
                </div>
                <div className="flex gap-2">
                  <input
                    type="checkbox"
                    checked={isTransUnionSelected}
                    onChange={(e) =>
                      handleCheckboxChange("transUnion", e.target.checked)
                    }
                  />
                  <p>TransUnion</p>
                </div>
                ...
                <div className="flex gap-2">
                  <input
                    type="radio"
                    id="differentForEach"
                    checked={isDifferentForEach}
                    onChange={handleDifferentForEach}
                  />
                  <p>Different for each bureau</p>
                </div>

                <div className={`${showDifferentFields ? "hidden" : "block"}`}>
                  <input
                    type="text"
                    value={accountnumber}
                ...
                <div
                  className={`${
                    showDifferentFields ? "flex" : "hidden"
                  } flex-col gap-2 mt-2`}
                >
                  <input
                    type="text"
                    value={equifaxStatus}
                    onChange={(e) => setEquifaxStatus(e.target.value)}
                    className="w-[90%] border-2 border-gray-400"
                    placeholder="Equifax Acc no"
                  />
                  <input
                    type="text"
                    value={experianStatus}
                    onChange={(e) => setExperianStatus(e.target.value)}
                    className="w-[90%] border-2 border-gray-400"
                    placeholder="Experian Acc no"
                  />
                  <input
                    type="text"
                    value={transUnionStatus}
                    onChange={(e) => setTransUnionStatus(e.target.value)}
                    className="w-[90%] border-2 border-gray-400"
                ...
                <button
                  className="w-[100px] h-[30px] bg-blue-500 mt-2 text-white"
